  ...
  chapterFits: [
    {
      ...
      mathFit: 'Total travel time crossing two surfaces is the sum of two radical-function terms:  t(x) = √((a−x)² + y₁²) / v₁  +  √(x² + y₂²) / v₂.  Each √(·) is a radical function of the boundary-crossing point x. Plug in real boardwalk and sand speeds from this dataset, plot t(x), and read the minimum off the graph — or solve t\'(x) = 0 algebraically. Students manipulate, evaluate, and graph two real radical functions, then add them.',
      standards: ['HSF-BF.A.1', 'HSF-IF.C.7.b', 'HSN-RN.A.2'],
      studentWhy: 'The math of "what\'s the fastest path?" is the same equation that bends light through water and decides where a lifeguard runs vs. swims. Each path length is a radical of position.',
      objective: 'Students will set up and minimize a sum of two radical functions modeling travel time across two surfaces with different speeds, using real speed data, then verify the optimal crossing point matches Snell\'s law of refraction.',
      minutes: 30,
      discussion: [
        'Write t(x) for boardwalk speed 9 m/s and sand speed 6.85 m/s, then graph it. Where is the minimum? What does that x value mean physically?',
        'Why does the sum-of-radicals form arise here? What does each radical represent?',
        'A lifeguard runs at 7 m/s on sand and swims at 1.5 m/s. Same two-radical equation. Where on the beach should they enter the water to reach a swimmer fastest?',
      ],
    },
  ],
  rows: RAW.map((r) => ({ ...r })),

  ...
  chapterFits: [
    {
      ...
      mathFit: 'The Savvas Act-1 is literal similarity — a town model with a Mayor figure too small, scaled up to match. Real-world version: this dataset gives you actual heights of 30 famous buildings. A 1:200 scale model of Burj Khalifa is 4.14 m tall (828 m / 200). A 1:500 model is 1.66 m. The scale factor IS the similarity ratio. Then the second move: floors-to-height across the dataset is approximately 4.5 m/floor (the proportionality constant of the "vertical similarity" between buildings) — letting students infer one dimension from the other via a single ratio.',
      standards: ['HSG-SRT.A.1', 'HSG-SRT.A.2', 'HSG-MG.A.1'],
      studentWhy: 'A 4-inch statue of the Mayor on a town model represents a real human in the same way a paper-clip skyscraper represents an 828-meter tower. The math is one ratio — and these are the buildings\' real heights.',
      objective: 'Students will use real building heights to compute scale factors for model-to-real similarity ratios, then verify floors-to-height as an approximately constant proportionality across modern supertalls.',
      minutes: 30,
      discussion: [
        'A 30-cm model of Burj Khalifa needs to use what scale? Compute real_height / model_height for a few buildings — when does the model fit on a tabletop?',
        'Floors × 4.5 m ≈ height. Which buildings break that rule (and how)? Modern supertalls have taller floor-to-floor heights — what does that say about scaling assumptions?',
        'The Mayor in the Savvas video is 4 inches tall on the model. If the model uses the same scale as a 30-cm Empire State (1:1270), how tall is the real Mayor?',
      ],
    },
  ],
  rows: RAW.map((r) => ({ ...r })),

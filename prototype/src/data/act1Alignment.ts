...
export const ALIGNMENT: AlignmentRow[] = [
  ...
  },
  {
    ...
    exploration: 'Four seafloor cross-sections — Hudson Canyon (a quadratic U), Cape Cod (peninsula with TWO real roots at the shorelines), Mauna Kea (volcanic mountain rising from −5000 m to +4207 m, one root at the shore), and the Mariana Trench (10,935 m deep — quartic captures the steep walls). Different students fitting the same bathymetry pick different polynomial degrees, but only the right factorization correctly locates the shoreline. The polynomial roots ARE the beach.',
    datasetIds: ['bathymetry'],
    strength: 'strong',
  },
  {
  ...
  },
  {
    ...
    exploration: '20 retro-game characters and what their engines do every frame. Pac-Man rotates by 90°; Mario reflects horizontally; Sonic composes translation + rotation; Tetris\'s O-piece has rotational symmetry order 4, while S and Z are chiral mirror twins. "Classification of Rigid Motions" is the literal subroutine table inside 1980s arcade ROMs — students can filter, sort by sprite symmetry, and trace transformations from 1978 (translation-only) to 1991 (full composition).',
    datasetIds: ['gameSprites'],
    strength: 'strong',
  },
  {
  ...
  },
  {
    ...
    exploration: 'The world\'s 30 tallest buildings — CTBUH-verified heights from 828 m (Burj Khalifa, 2010) to 428 m (Shandong IFC, 2025). The Savvas Act-1 scales a town model up to the real Mayor: same proportional reasoning powers a 1:500 architectural model of the Burj Khalifa (1.66 m tower; 1 cm per floor) up to the 828 m real building. The dataset also exposes between-building design differences — Empire State runs 3.7 m/floor, Burj Khalifa runs 5 m/floor — that proportional reasoning makes visible.',
  ...
  },
  {
    ...
    exploration: 'Real lidar-revealed structures at Caracol (Maya, Belize — 2009 NASA campaign that exposed 40,000 hidden Angkor features in a follow-up), Angkor (Khmer, Cambodia), and the Mosquitia "Lost City" (Honduras, 2015). Each temple has an (x, y) coordinate. The midpoint of two ceremonial centres is where the sacbe causeway passes — at Caracol, excavators dug at a predicted midpoint and found a third plazuela cluster within 50 m. Midpoint math = real archaeological hypothesis.',
    datasetIds: ['lidarRuins'],
    strength: 'strong',
  },
  {
  ...
  },
  {
    ...
    exploration: '12 running surfaces with real sprint speeds from peer-reviewed sports-medicine studies. Synthetic track is 9.45 m/s; deep sand is 5.80 m/s (39% slower). Two paths from beach blanket to snack shack cross sand and boardwalk: path length on each leg is a direct radical, √((D−x)² + W²) on the soft side and √(x² + L²) on the hard side. Students compute and graph these radical expressions; real surface speeds make the math concrete without slipping into calculus-only territory.',

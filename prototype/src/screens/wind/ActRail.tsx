        ...
        return (
          <div key={a.num} className="flex flex-col items-start">
              ...
              <div className="min-w-0">
                <div
                  className={`text-xs font-bold tracking-widest ${
                    state === 'active' ? 'text-sky-700' : 'text-slate-400'
                  }`}
                >
                  {a.label.toUpperCase()}
                </div>
            ...
            {state === 'active' && step && stepLabels && (
              <ol className="ml-[23px] mt-4 border-l-2 border-sky-200 pl-5 space-y-3">
            ...
            {!isLast && (
              <div
                ...
                className={`w-0.5 h-12 ml-[23px] my-2 rounded-full ${
                  state === 'done' ? 'bg-sky-300' : 'bg-slate-200'
                }`}
              />
            )}
          </div>
...
function Dot({ state, num }: { state: State; num: number }) {
  if (state === 'active') {
    return (
      <div className="grid place-items-center w-12 h-12 rounded-full bg-gradient-to-br from-sky-600 to-blue-600 text-white text-base font-display font-bold shadow-md ring-4 ring-sky-100 shrink-0">
        {num}
      </div>
    );
  }
  if (state === 'done') {
    return (
      <div className="grid place-items-center w-12 h-12 rounded-full bg-sky-500 text-white text-base font-display font-bold shrink-0">
        ✓
      </div>
    );
  }
...
function StepDot({ state, num }: { state: State; num: number }) {
  if (state === 'active') {
    return (
      <span className="grid place-items-center w-7 h-7 rounded-full bg-sky-600 text-white text-xs font-display font-bold shrink-0 ring-2 ring-sky-100">
        {num}
      </span>
    );
  }
  if (state === 'done') {
    return (
      <span className="grid place-items-center w-7 h-7 rounded-full bg-sky-200 text-sky-700 text-xs font-display font-bold shrink-0">
        ✓
      </span>
    );
  }

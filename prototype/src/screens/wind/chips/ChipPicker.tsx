// Shared chip-group component for the Wind Power Curve activity. Renders a
...
interface ChipPickerProps {
  ...
  max?: number;
  /** Tone for the active state. 'sky' for everyday, 'amber' for synthesis. */
  tone?: 'sky' | 'amber';
}
...
export default function ChipPicker({
  ...
  const activeClasses =
    tone === 'amber'
      ...
      : 'bg-gradient-to-br from-sky-600 to-blue-600 text-white border-sky-600 shadow-md ring-2 ring-sky-200';
            ...
            <button
              ...
              className={`group/chip inline-flex items-center gap-2 text-left px-4 py-2.5 rounded-2xl text-sm font-semibold leading-snug transition-all border-2 ${
                active
                  ...
                  : 'bg-white text-slate-800 border-slate-200 hover:border-sky-300 hover:bg-sky-50 hover:-translate-y-0.5 hover:shadow-sm active:translate-y-0'
              }`}
            >
    ...
    <span
      className={`grid place-items-center w-5 h-5 rounded-full text-base leading-none font-light shrink-0 transition-colors ${
        tone === 'amber'
          ...
          : 'bg-sky-50 text-sky-600 group-hover/chip:bg-sky-100 group-hover/chip:text-sky-700'
      }`}
    >
      +
    </span>

...
export const NARRATOR = {
  name: 'Maya Chen',
  role: 'Music Data Analyst · Streaming Platform',
...
function startLine(key: NarratorLineKey) {
  stopCurrent();
  const audio = new Audio(`/audio/spotify-narrator/${key}.mp3`);
  audio.onended = () => {
    if (currentKey === key) {
      currentKey = null;
      currentAudio = null;
  ...
  audio.onerror = () => {
    if (currentKey === key) {
      ...
      notify();
      console.warn(
        `Narrator audio missing: /audio/spotify-narrator/${key}.mp3`,
      );
    }
  };
  currentAudio = audio;
...
// --------------------------------------------------------------------------
// Visual components — pink accent palette matching the Spotify theme
// --------------------------------------------------------------------------
...
export function NarratorAvatar({ size = 64 }: { size?: number }) {
  ...
  return (
    <img
      ...
      className="shrink-0 rounded-full object-cover ring-1 ring-pink-200/60"
...
function NarratorAvatarFallback({ size }: { size: number }) {
  return (
    ...
    >
      ...
      {/* long dark hair */}
      <path d="M22,44 Q22,18 50,17 Q78,18 78,44 Q76,36 50,35 Q24,36 22,44 Z" fill="#1a1a2e" />
      <path d="M22,44 Q18,55 16,80 L22,80 Q24,60 26,56 Z" fill="#1a1a2e" />
      <path d="M78,44 Q82,55 84,80 L78,80 Q76,60 74,56 Z" fill="#1a1a2e" />
      ...
      {/* brows */}
      <path d="M37,46 Q41,44 45,46" stroke="#1a1a2e" strokeWidth="1.5" fill="none" strokeLinecap="round" />
      <path d="M55,46 Q59,44 63,46" stroke="#1a1a2e" strokeWidth="1.5" fill="none" strokeLinecap="round" />
      {/* headphones */}
      <path d="M28,44 Q28,28 50,26 Q72,28 72,44" fill="none" stroke="#ec4899" strokeWidth="3.5" />
      <ellipse cx="28" cy="46" rx="6" ry="7" fill="#ec4899" opacity={0.85} />
      <ellipse cx="72" cy="46" rx="6" ry="7" fill="#ec4899" opacity={0.85} />
      {/* smile */}
  ...
  const palette =
    tone === 'dark'
      ? {
          ...
          active: 'bg-pink-300 text-slate-900 border-pink-300 shadow',
        }
      : {
          idle: 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-pink-50 hover:border-pink-300 hover:text-pink-700',
          active: 'bg-pink-600 text-white border-pink-600 shadow',
        };
  return (
    <button
      onClick={toggle}
...
export function NarratorIntro() {
  return (
    <div className="bg-gradient-to-br from-pink-50 to-purple-50 border border-pink-200 rounded-2xl p-5 sm:p-6 shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline justify-between gap-2 flex-wrap">
            <div>
              ...
              <div className="text-xs font-semibold text-pink-700">{NARRATOR.role}</div>
            </div>
  ...
  return (
    <div className="bg-white/5 backdrop-blur border border-white/15 rounded-xl p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline justify-between gap-2 flex-wrap">
            <div className="text-[10px] font-semibold tracking-widest text-pink-300 leading-tight">
              {NARRATOR.name.toUpperCase()} · {NARRATOR.org}
            </div>
